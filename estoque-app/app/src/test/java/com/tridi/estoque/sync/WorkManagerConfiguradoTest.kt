...
class WorkManagerConfiguradoTest {
    ...
    fun `removendo o inicializador padrao, a Application precisa fornecer a configuracao`() {
        ...
        val application = ler("src/main/java/com/tridi/estoque/EstoqueApplication.kt")
        ...
        if (removeOInicializador) {
            assertTrue(
                "O manifesto remove o WorkManagerInitializer mas a EstoqueApplication não " +
    ...
    fun `ninguem volta a inicializar o WorkManager na mao`() {
        ...
        val agendador = ler("src/main/java/com/tridi/estoque/sync/EstoqueWorkScheduler.kt")
        assertTrue(
            "EstoqueWorkScheduler voltou a chamar WorkManager.initialize — isso só cobre " +
