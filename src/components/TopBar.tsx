...
export function TopBar({ username, onSettingsClick }: TopBarProps) {
  return (
    <header className="sticky top-0 z-20 px-4 py-3 flex items-center justify-center" style={{ backgroundColor: '#2D1169' }}>
      <h1 className="text-[17px] font-semibold text-white tracking-tight flex-1 text-center">
        {username}
      </h1>
      <button
        onClick={onSettingsClick}
        className="absolute right-4 top-1/2 -translate-y-1/2 p-1 text-white hover:text-white transition-colors"
